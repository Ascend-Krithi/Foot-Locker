...
public class StoreResultsPage {
    ...
    public void clickSetMyStoreForAddress(String expectedAddress) {
        List<WebElement> stores = WaitUtils.waitForElementsToBeVisible(driver, storeCards);
        for (WebElement store : stores) {
            try {
                WebElement addressElement = store.findElement(storeAddress);
    ...
    public boolean isMyStoreConfirmationDisplayed() {
        By myStoreConfirmation = By.xpath("//*[contains(.,'My Store') or contains(.,'Your Store')]");
        return BrowserUtils.isDisplayed(driver, myStoreConfirmation);
    }
}