...
/**
 * TC_ID: 4122
 * Test Case: Onboard Installer With Minimum Certifications
 * Description: Login as admin, navigate to installer applications, select application with minimum certifications (MCS), review/vet, approve, verify installer onboarded.
 */
public class TS009_TC001_OnboardInstallerWithMinimumCertifications extends BaseTest {

    @Test
    public void onboardInstallerWithMinimumCertifications() {
        ...
        try {
            ...
            WebElement mcsApplication = wait.until(ExpectedConditions.presenceOfElementLocated(
                By.xpath("//*[contains(translate(.,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'mcs')]")
            ));
            clickElement(mcsApplication);
            ...
            Assert.assertTrue(successMessage.isDisplayed(), "Installer with minimum certifications not onboarded");
        } catch (Exception e) {
            Assert.fail("Onboard installer with minimum certifications failed: " + e.getMessage());
        }
    }
